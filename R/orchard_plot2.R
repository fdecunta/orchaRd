...
#' @param yaxis_angle The angle of y labels. The default is 90 degrees.
#' @param color_blind If \code{TRUE}, it uses 20 colour blind friendly colors.
...
orchard_plot2 <- function(
  ...
  if (!is.null(tree.order)) {
    mod_result <- .order_tree(mod_result, tree.order)
  }

  mod_table <- mod_result$mod_table
  data_trim <- mod_result$data
  ...
  plt <- .base_orchard_plot(data_trim, colour, fill, alpha) +
    ...
    .orcd_colour_blind_palette(color_blind) +
    ggplot2::coord_flip()
...
#'
#' @keywords internal
.orcd_colour_blind_palette <- function(color_blind) {
  if (color_blind) {
    return(
      list(
