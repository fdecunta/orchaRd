...
mod_results <- function(model,
  ...
  if (is_categorical(mod_vector)) {
    ...
    at2 <- list(mod = seq(min(mod_vector, na.rm = TRUE),
                          ...
                          length.out = 100))
    names(at2) <- mod
    grid_args$at <- c(at2, at)  # Getting 100 points. Fixing this to make it more general
  }
...
#' Validate 'model'
#'
#' This function checks whether the supplied `model` is a valid metafor model object.
...
#' @title mod_is_valid
#' @description
#' Checks whether the specified `mod` argument is valid. This argument must be either
...
#' Validate 'group'
#'
#' Checks whether the specified grouping variable is valid within the model's dataset. 
#' The function ensures that the `group` argument is provided, exists as a column 
